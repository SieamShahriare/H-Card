...
export default function SelectDoctor({ setScreen }: { setScreen: (s: Screen) => void }) {
  return (
    <div className="space-y-6 pb-8">
      <section className="pt-4">
        <span className="text-secondary font-bold uppercase tracking-widest text-[10px]">Appointment Flow</span>
        <h1 className="text-3xl font-extrabold tracking-tight text-primary leading-tight mt-2 mb-2">Cardiologists at Evercare Hospital</h1>
      ...
      <div className="flex flex-col gap-6">
        <DoctorCard name="Dr. Anisur Rahman" role="Senior Consultant" rating="4.9" reviews="120+" nextAvailable="Tomorrow" time="09:00 AM" onClick={() => setScreen('schedule-appointment')} />
        <DoctorCard name="Dr. Sarah Jahan" role="Associate Professor" rating="4.8" reviews="95+" nextAvailable="Today" time="04:30 PM" onClick={() => setScreen('schedule-appointment')} />
        <DoctorCard name="Dr. Michael Chen" role="Department Head" rating="5.0" reviews="210+" nextAvailable="Wed, Oct 25" time="10:00 AM" onClick={() => setScreen('schedule-appointment')} />
      </div>
    </div>
  );
}

function DoctorCard({ name, role, rating, reviews, nextAvailable, time, onClick }: any) {
  return (
    <div className="bg-surface-container-lowest rounded-3xl p-5 shadow-sm border border-outline-variant/10 flex flex-col gap-5">
      ...
      <div className="flex items-center justify-between bg-surface-container-low px-4 py-3 rounded-2xl">
        <div className="flex items-center gap-2">
          ...
          <span className="text-xs font-semibold text-on-surface">Next available: <span className="text-emerald-700">{nextAvailable}</span></span>
        </div>
