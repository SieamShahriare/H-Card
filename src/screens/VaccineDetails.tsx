    ...
    <div className="space-y-10 pb-8">
      ...
      <section className="space-y-6">
        <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-emerald-100 text-primary text-[10px] font-bold tracking-wider uppercase">
          ...
          {vaccine.status === 'Completed' ? 'Verified Record' : 'Recommended'}
        </div>
        ...
        <div className="flex flex-wrap gap-4">
          <div className="flex flex-col p-4 bg-surface-container-low rounded-2xl min-w-[140px]">
            ...
            <span className="text-xl font-bold text-primary">{vaccine.doses}</span>
          </div>
          <div className="flex flex-col p-4 bg-surface-container-low rounded-2xl min-w-[140px]">
            <span className="text-[9px] font-bold text-outline uppercase tracking-widest mb-1">Effectiveness</span>
            <span className="text-xl font-bold text-primary">{vaccine.effectiveness}</span>
          </div>
        </div>
      </section>
          ...
          <div className="flex items-start gap-4 mb-6">
            ...
            <div>
              ...
              <p className="text-on-surface-variant mt-2 text-sm">
                Key benefits of maintaining your {vaccine.title} immunization status.
              </p>
            </div>
          </div>
          
          <ul className="space-y-4">
            {vaccine.benefits.map((benefit, i) => (
          ...
          <div className="p-8 bg-primary text-white rounded-3xl shadow-xl flex flex-col justify-between min-h-[200px]">
            <div>
              ...
              <p className="text-emerald-100/70 text-sm mb-6 leading-relaxed">
                Schedule your next dose today at our nearest partner hospital for verified records.
              </p>
            </div>
          ...
          <div className="flex justify-between items-end">
            <div>
              <span className="text-[10px] font-bold text-secondary uppercase tracking-[0.2em]">
                {vaccine.status === 'Completed' ? 'ADMINISTERED AT' : 'NEAREST PROVIDER'}
              </span>
              <h3 className="text-2xl font-extrabold text-on-surface tracking-tight">{hospital.name}</h3>
            </div>
            ...
            <div className="absolute bottom-4 left-4 right-4 p-4 bg-white/90 backdrop-blur-xl rounded-2xl shadow-2xl border border-white/50">
              <div className="flex gap-4">
                ...
                <div className="flex-1">
                  <h4 className="font-bold text-on-surface text-xs">{hospital.name}</h4>
                  <p className="text-on-surface-variant text-[10px] mt-1">{hospital.address}</p>
                  <div className="flex items-center gap-2 mt-2">
                    <span className="px-2 py-0.5 bg-emerald-100 text-emerald-700 text-[8px] font-bold rounded uppercase">{hospital.status}</span>
